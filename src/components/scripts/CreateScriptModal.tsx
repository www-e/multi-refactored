...
import { SCRIPT_CATEGORIES } from '@/app/(shared)/types';
...
export function CreateScriptModal({ onClose, onSubmit }: CreateScriptModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'فشل في إنشاء السكريبت. يرجى المحاولة مرة أخرى.');
    } finally {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">إنشاء سكريبت جديد</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">قم بإنشاء سكريبت مخصص للحملات</p>
            </div>
          </div>
          <button
            onClick={onClose}
